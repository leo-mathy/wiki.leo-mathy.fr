...
| Type | Description|
| -- | -- |
| Serveur DNS Racine | Les serveurs racines DNS sont responsables des domaines de premier niveau (TLD). En dernière instance, ils sont uniquement intérogés si le serveur de noms de réponds pas. C'est l'interface centrale entre les utilisateurs et le contenu sur Internet. C'est l'ICANN qui coordonne les serveurs racines. Il y a 13 serveur racines dans le monde. |
| | |
| | |
| | |
| | |
